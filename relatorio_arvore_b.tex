...
\textbf{Ordenação das chaves:} Manter as chaves ordenadas dentro de cada nó foi um desafio, pois diversas situações podiam ocorrer ao inserir um novo elemento — no início, no meio ou no final. Para resolver isso, foi criada a função auxiliar \texttt{add\_key()}, que centraliza e trata todas essas situações.
